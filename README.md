...
- **Embedded shell** — a shared terminal core over a bundled xterm.js 6.0 renderer with the official fit, unicode11, search, web-links and WebGL addons. Text uses the crisp DOM renderer by default; **Layout (F11) → Terminal compatibility (next launch) → GPU renderer** switches to WebGL for very high output throughput. Font family (any installed fixed-pitch family) and size are configurable from the dock's **⋯** menu and persist across restarts; the default is **Consolas at 14px** with Cascadia Mono and platform fallbacks. PTY support, prompt-aware directory following, highlighted search, zoom, copy/paste, and pinned commands. **Terminal compatibility** also selects standard or classic controls without replacing the current session. Claude Code uses the same renderer and process backend while keeping its separate per-folder sessions.
...
- **Transfer progress** shows action counts plus bytes, speed, and estimated time remaining for the current item. Cancellation checks run between copy blocks and archive members. Clean finishes close the progress window automatically; failures keep it open with details and everything is kept in the session **Log** (the button beside Folders / Thumbnails in the active pane). Closing the app requests cancellation and waits for active file jobs to finish safely.
