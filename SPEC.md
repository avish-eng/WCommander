...
### 14.1 Command Bar (Removed)

The inline command bar was dropped. The embedded terminal (`F9`) covers command execution with a real PTY, searchable history, and prompt-aware directory following, so a second, line-oriented command entry point is not needed. `Ctrl+G` is unassigned.
