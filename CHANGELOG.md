...
- F3 Quick View renders broader file types inline. Beyond the existing image and plain-text branches, the passive-pane preview now handles Markdown (`.md`, `.markdown`) via `QTextBrowser.setMarkdown`, HTML (`.html`, `.htm`) via `setHtml`, PDF (`.pdf`) via `QPdfView` + `QPdfDocument` with page navigation, SVG (`.svg`) via `QSvgWidget`, and syntax-highlighted source for any language `pygments` recognises by filename (Python, TypeScript, JSON, etc. via `HtmlFormatter(noclasses=True, nobackground=True)`). Binary files now show a classic offset/hex/ascii dump of the first 4 KB instead of the "Binary preview is not available yet" placeholder.
